...
| 评分进度 | `576 / 1000` | `docs/tracking/SCORE_TRACKING.md` |
...
| Serial Station | UART/TCP/UDP 主流程已有可见入口；TCP/UDP 端点校验、连接/发送、导出/回放/Profile 和 RX 注入状态消息已公共化，UDP UI loopback 收发已验证，断开、刷新端口、发送、快捷键、命令历史、Profile 恢复、错误状态反馈和窗口生命周期可诊断 | `tests/python/ui_smoke/test_serial_station_udp_ui.py` |
