/**
 * @file FireWaterBridgeParsing.cpp
 * @brief FireWater协议桥 — 行解析核心逻辑
 *
 * 从 FireWaterBridge.cpp 拆分而来，包含:
 *   - parseLines(): 从缓冲区提取所有完整行并逐行解析
 *   - processLine(): 单行处理(头部识别/通道名初始化/数据解析)
 *   - parseValueLine(): 将CSV数据行解析为通道名→浮点值映射
 *
 * 接口实现(feed/reset/name/configure)保留在 FireWaterBridge.cpp。
 * 统计getter/resetStatistics见 FireWaterBridgeStats.cpp。
 */
