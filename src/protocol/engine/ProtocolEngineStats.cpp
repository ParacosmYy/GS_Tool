...
/** @brief 获取最后一次成功解析的时间戳 @return 毫秒级时间戳，尚未解析过时返回0 */
qint64 ProtocolEngine::lastParseTimestamp() const
{
    return m_lastParseTimestamp;
}

/** @brief 获取已处理的数据包总数(成功+失败) @return 成功解析帧数+被拒绝帧数 */
