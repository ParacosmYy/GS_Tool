 ...
 * @brief 仪表盘布局序列化器实现 — JSON读写+文件操作+验证
 *
 * 支持gauge/numeric/led/progressbar/chart五种面板类型的
 * JSON文件保存/加载、布局验证和文件管理。
 *
 * 统计查询方法见 DashboardSerializerStats.cpp
 * JSON序列化/反序列化见 DashboardSerializerExport.cpp
    ...
    const QList<DashboardItemConfig>& items, int columns) const
{
    ...
    for (int i = 0; i < items.size(); ++i) {
        for (int j = i + 1; j < items.size(); ++j) {
            ...
            bool rowOv = (a.row < b.row + b.rowSpan) && (b.row < a.row + b.rowSpan);
            bool colOv = (a.column < b.column + b.columnSpan) && (b.column < a.column + b.columnSpan);
            if (rowOv && colOv)
