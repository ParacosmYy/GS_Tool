...
public:
    explicit WebSocketConnection(QObject* parent = nullptr); ///< 构造WebSocket连接
    ~WebSocketConnection() override;                         ///< 析构，关闭连接并释放资源
    // ---- IConnection接口 ----
    ...
    qint64 write(const QByteArray& data) override;     ///< 发送二进制数据
    void configure(const QVariantMap& params) override; ///< 配置WebSocket参数
    // ---- WebSocket特有接口 ----
    ...
    // ---- 统计接口 ----
    quint64 totalConnections() const;       ///< 获取累计WebSocket连接成功次数
    quint64 totalMessagesSent() const;      ///< 获取已发送消息总数(文本+二进制)
    quint64 totalMessagesReceived() const;  ///< 获取已接收消息总数(文本+二进制)
    quint64 totalBytesSent() const;         ///< 获取已发送字节总数(帧级别)
    quint64 totalBytesReceived() const;     ///< 获取已接收字节总数(帧级别)
    quint64 errorCount() const;             ///< 获取错误计数
    quint64 totalFramesSent() const;        ///< 获取已发送帧总数(含所有opcode)
    quint64 totalFramesReceived() const;    ///< 获取已接收帧总数(含所有opcode)
    quint64 totalTextFrames() const;        ///< 获取已发送文本帧总数
    quint64 totalBinaryFrames() const;      ///< 获取已接收二进制帧总数
    quint64 totalPingFrames() const;        ///< 获取已发送ping帧总数
    quint64 totalPongFrames() const;        ///< 获取已接收pong帧总数
    quint64 totalFragmentedMessages() const;///< 获取已接收分片消息总数
    quint64 totalHandshakeAttempts() const { return m_totalHandshakeAttempts; } ///< 获取累计握手尝试次数
    quint64 totalHandshakeFailures() const { return m_totalHandshakeFailures; } ///< 获取累计握手失败次数
    quint64 totalCloseFramesSent() const { return m_totalCloseFramesSent; }     ///< 获取累计发送close帧次数
    quint64 totalCloseFramesReceived() const { return m_totalCloseFramesReceived; } ///< 获取累计接收close帧次数
    quint64 pingPongCount() const;          ///< 获取ping/pong交互总次数
    double averageLatencyMs() const;        ///< 获取ping/pong平均延迟(ms)
    qint64 maxLatencyMs() const;            ///< 获取ping/pong最大延迟(ms)
    qint64 connectionUptimeSeconds() const; ///< 获取当前连接运行时长(秒)
    int messageQueueSize() const;           ///< 获取当前消息队列大小
    int messageQueueLimit() const;          ///< 获取消息队列容量上限
    /** @brief 设置队列容量上限(<=0不限) @param limit 队列上限 */
    void setMessageQueueLimit(int limit);
    quint64 totalMessagesDropped() const;   ///< 获取因队列满而丢弃的消息数
    quint64 totalReconnects() const { return m_totalReconnects; }         ///< 获取累计重连次数
    quint64 totalTextFramesReceived() const { return m_totalTextFramesReceived; } ///< 获取累计接收文本帧总数
    quint64 totalBinaryFramesReceived() const { return m_totalBinaryFramesReceived; } ///< 获取累计接收二进制帧总数
    void resetStats();                      ///< 重置所有统计

signals:
    void textMessageReceived(const QString& message);   ///< 收到文本消息 @param message 文本内容
    void binaryMessageReceived(const QByteArray& data); ///< 收到二进制消息 @param data 二进制数据
    void pongReceived(const QByteArray& payload);       ///< 收到pong响应 @param payload pong载荷
