...
#include "utils/signal77/OnsetDetector3.h"
 ...
 * @param parent 父对象指针
 */
OnsetDetector3::OnsetDetector3(QObject* parent)
    : QObject(parent)
{
}
 ...
 * @param sr 采样率(Hz)
 */
void OnsetDetector3::setSampleRate(double sr)
{
 ...
 * @param ms 攻击时间(ms)
 */
void OnsetDetector3::setAttackTime(double ms)
{
 ...
 * @param ms 释放时间(ms)
 */
void OnsetDetector3::setReleaseTime(double ms)
{
 ...
 */
void OnsetDetector3::setMethod(const QString& method)
{
 ...
 * 然后应用攻击/释放平滑得到最终包络曲线。
 */
QVector<double> OnsetDetector3::detect(const QVector<double>& signal)
{
    QElapsedTimer timer;
    timer.start();
 ...
 * @brief 重置统计信息
 */
void OnsetDetector3::resetStatistics()
{
    m_stats = Stats();
    m_timeSum = 0.0;
}
 ...
 */
QVector<double> OnsetDetector3::hilbertEnvelope(const QVector<double>& sig)
{
 ...
 * 信号上升时快速跟踪，下降时缓慢释放。
 */
QVector<double> OnsetDetector3::peakEnvelope(const QVector<double>& sig)
{
    const int N = sig.size();
    QVector<double> env(N, 0.0);
