...
public:
    ...
    /** @brief 构造数据格式转换引擎 @param parent 父对象 */
    explicit DataConverter(QObject *parent = nullptr);

    /** @brief 格式转换 @param input 输入数据 @param from 源格式 @param to 目标格式 @return 转换后的数据 */
    QByteArray convert(const QByteArray &input, Format from, Format to) const;

    /** @brief 自动检测数据格式 @param data 待检测数据 @return 检测到的最可能格式 */
    Format detectFormat(const QByteArray &data) const;

    /** @brief 获取格式名称 @param format 格式枚举值 @return 格式名称字符串 */
    static QString formatName(Format format);

    /** @brief 获取格式的人类可读描述 @param format 格式枚举值 @return 描述字符串，适合作为UI工具提示 */
    static QString formatDescription(Format format);

    /** @brief 获取所有支持的格式列表 @return 格式枚举列表 */
    static QList<Format> supportedFormats();

    /** @brief 将输入数据转换到所有其他格式(对比视图) @param input 输入数据 @param from 源格式 @return QMap<格式名,转换结果> */
...
private:
    /** @brief 将数据从指定格式解码为原始字节 @param input 输入数据 @param from 源格式 @return 原始字节数组 */
    QByteArray decodeToRaw(const QByteArray &input, Format from) const;

    /** @brief 将原始字节编码为指定格式 @param raw 原始字节 @param to 目标格式 @return 编码后的数据 */
    QByteArray encodeFromRaw(const QByteArray &raw, Format to) const;

    mutable quint64 m_convCount = 0;              ///< 累计转换次数
