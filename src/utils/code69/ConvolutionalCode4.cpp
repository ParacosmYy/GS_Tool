...
#include "utils/code69/ConvolutionalCode4.h"
 ...
 */
void ConvolutionalCode4::setGenerators(const QVector<int>& gens)
{
    if (!gens.isEmpty()) {
        m_gens = gens;
    }
 ...
 */
void ConvolutionalCode4::setConstraintLength(int k)
{
 ...
 * @return 解码后的硬判决比特
 */
QVector<int> ConvolutionalCode4::decode(const QVector<double>& softBits)
{
    QElapsedTimer timer;
    timer.start();
 ...
 */
int ConvolutionalCode4::freeDistance() const
{
    // 简化：通过遍历短序列估计自由距离
    int numStates = 1 << (m_constraint - 1);
    int minDist = 100;
