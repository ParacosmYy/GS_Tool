...
#include "utils/graph89/SteinerTree4.h"
 ...
 * @param parent 父对象指针
 */
SteinerTree4::SteinerTree4(QObject* parent)
    : QObject(parent)
{
}
 ...
 */
void SteinerTree4::setRooted(bool rooted)
{
    m_rooted = rooted;
}
 ...
 * @param n 顶点数
 */
void SteinerTree4::setVertexCount(int n)
{
    m_n = qMax(0, n);
    m_adj.clear();
    m_adj.resize(m_n);
 ...
 * 便于后续标记已使用的边。
 */
void SteinerTree4::addEdge(int u, int v)
{
 ...
 * 3. DFS搜索扩展回路/路径
 */
QVector<int> SteinerTree4::compute()
{
    QElapsedTimer timer;
    timer.start();
 ...
 * @brief 重置统计信息
 */
void SteinerTree4::resetStatistics()
{
    m_stats = Stats();
    m_timeSum = 0.0;
}
 ...
 * 最后反转路径得到欧拉回路。
 */
void SteinerTree4::findEulerTour(int start, QVector<int>& tour, QVector<bool>& used)
{
    QVector<int> stack;
    stack.append(start);
