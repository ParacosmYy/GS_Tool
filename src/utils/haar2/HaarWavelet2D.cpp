 ...
 * @return 变换系数矩阵
 */
QVector<QVector<double>> HaarWavelet2D::forward(
 ...
 * @return 重构后的二维数据
 */
QVector<QVector<double>> HaarWavelet2D::inverse(
...
void HaarWavelet2D::setLevels(int levels)
{
 ...
 */
void HaarWavelet2D::transformRows(QVector<QVector<double>>& data) const
{
    int rows = data.size();
 ...
 */
void HaarWavelet2D::transformCols(QVector<QVector<double>>& data) const
{
    int cols = data[0].size();
 ...
 * @brief 单级行逆变换
 */
void HaarWavelet2D::inverseRows(QVector<QVector<double>>& data) const
{
    int rows = data.size();
    int halfCols = data[0].size() / 2;
 ...
 * @brief 单级列逆变换
 */
void HaarWavelet2D::inverseCols(QVector<QVector<double>>& data) const
{
    int cols = data[0].size();
    int halfRows = data.size() / 2;
...
/** @brief 检查是否为2的幂 */
bool HaarWavelet2D::isPowerOfTwo(int n)
{
    return n > 0 && (n & (n - 1)) == 0;
}

/** @brief 向上取整到2的幂 */
int HaarWavelet2D::nextPowerOfTwo(int n)
{
    if (n <= 0) return 1;
    int p = 1;
    while (p < n) p *= 2;
 ...
 * 右侧补零列，底部补零行
 */
QVector<QVector<double>> HaarWavelet2D::padToPowerOfTwo(
