...
public:
    ...
    enum TimeInterval {
        Interval_1s = 1, Interval_5s = 5, Interval_30s = 30, Interval_1min = 60, Interval_5min = 300
    };
    Q_ENUM(TimeInterval)
    /** @brief 时间窗口统计结果结构体 */
    struct WindowStats {
    ...
    // ---- 滑动窗口聚合 ----
    /** @brief 添加数据源并指定聚合函数和窗口大小 @param name 数据源名称 @param func 聚合函数类型 @param windowSize 滑动窗口大小(样本数) */
    ...
    /** @brief 向指定数据源输入一个新值 @param source 数据源名称 @param value 输入值 */
    ...
    /** @brief 重置指定数据源 @param source 数据源名称 */
    void resetSource(const QString &source);
    void resetAll();                                      ///< 重置所有数据源的缓冲区和结果
    // ---- 时间窗口聚合 ----
    /** @brief 为数据源启用时间窗口聚合 @param name 数据源名称 @param intervalSec 窗口间隔(秒) */
    ...
    /** @brief 获取所有已完成时间窗口统计 @param name 数据源名称 @return 窗口统计列表(时间升序) */
    ...
    // ---- 实时滚动聚合 ----
    /** @brief 启用实时滚动聚合定时器 @param rollIntervalMs 滚动间隔(毫秒)，默认1000 */
    ...
    // ---- 统计访问器 ----
    quint64 totalValuesFed() const { return m_totalValuesFed; }             ///< 获取累计输入值总数
    quint64 totalWindowsCompleted() const { return m_totalWindowsCompleted; }///< 获取累计完成的时间窗口数
    quint64 totalRollingEmits() const { return m_totalRollingEmits; }      ///< 获取累计滚动聚合发射次数
    quint64 totalSourceAdds() const { return m_totalSourceAdds; }          ///< 获取累计数据源添加次数
    quint64 totalSourceRemoves() const { return m_totalSourceRemoves; }    ///< 获取累计数据源移除次数
    quint64 totalResets() const { return m_totalResets; }                  ///< 获取累计重置次数
    quint64 totalTimeWindowEnables() const { return m_totalTimeWindowEnables; } ///< 获取累计时间窗口启用次数
    quint64 totalTimeWindowDisables() const { return m_totalTimeWindowDisables; }///< 获取累计时间窗口禁用次数
    quint64 totalWindowSlides() const { return m_totalWindowSlides; }     ///< 获取累计滑动窗口淘汰次数
    quint64 totalOutputEmissions() const { return m_totalOutputEmissions; }///< 获取累计聚合结果发射次数
    quint64 totalComputeCalls() const { return m_totalComputeCalls; }     ///< 获取累计聚合计算调用次数
...
private:
    void computeAggregate(const QString &source);          ///< 执行滑动窗口聚合计算
    /** @brief 将新值纳入时间窗口统计 @param source 数据源名称 @param value 新值 @param timestampMs 时间戳 */
    ...
    /** @brief 计算时间窗口起始时间 @param timestampMs 时间戳(毫秒) @param intervalSec 窗口间隔(秒) @return 窗口起始时间(毫秒) */
    ...
    quint64 m_totalWindowSlides = 0;       ///< 累计滑动窗口淘汰次数
    quint64 m_totalOutputEmissions = 0;    ///< 累计聚合结果发射次数
    quint64 m_totalComputeCalls = 0;       ///< 累计聚合计算调用次数
};
