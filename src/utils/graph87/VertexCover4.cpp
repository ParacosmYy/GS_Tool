 ...
 * @param n 左侧顶点数量
 */
void VertexCover4::setLeftSize(int n)
{
    m_left = qMax(0, n);
}
 ...
 * @param m 右侧顶点数量
 */
void VertexCover4::setRightSize(int m)
{
    m_right = qMax(0, m);
}
 ...
 * @return 最大匹配的边数
 */
int VertexCover4::maxCardinality()
{
    QElapsedTimer timer;
    timer.start();
 ...
 * @return 匹配边列表
 */
QVector<QPair<int,int>> VertexCover4::maxCardinalityMatching()
{
 ...
 */
QVector<int> VertexCover4::vertexCover()
{
    QVector<int> cover;
 ...
 * @return 是否找到增广路径
 */
bool VertexCover4::augment(int u, QVector<bool>& visited, QVector<int>& matchR)
{
    for (int v : m_adj[u]) {
