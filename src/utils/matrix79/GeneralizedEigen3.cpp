...
#include "utils/matrix79/GeneralizedEigen3.h"
 ...
 * @param A 输入矩阵，m行n列
 */
void GeneralizedEigen3::setMatrix(const QVector<QVector<double>>& A)
{
    if (A.isEmpty()) return;
    m_rows = A.size();
    m_cols = A[0].size();
 ...
 */
bool GeneralizedEigen3::decompose()
{
    QElapsedTimer timer;
    timer.start();
 ...
 */
QVector<double> GeneralizedEigen3::solve(const QVector<double>& b)
{
    QElapsedTimer timer;
    timer.start();
 ...
 */
double GeneralizedEigen3::conditionNumber() const
{
 ...
 */
int GeneralizedEigen3::effectiveRank(double tol) const
{
