...
struct RedBlackTree8::Node {
    int key;               ///< 节点键值
    QVariant value;        ///< 节点存储数据
    bool isRed = true;     ///< 颜色：true=红色, false=黑色
    Node* left = nullptr;
    Node* right = nullptr;
    Node* parent = nullptr;
 ...
 */
RedBlackTree8::RedBlackTree8(QObject* parent) : QObject(parent) {}

/**
 * @brief 插入键值对到红黑树
 *
 * BST插入后通过重新着色和旋转恢复红黑性质。
 * 新节点着红色插入，若父节点为红色则根据叔节点
 * 颜色选择重着色或旋转操作。
 *
 * @param key 插入的键
 * @param value 关联的值
...
void RedBlackTree8::insert(int key, const QVariant& value)
{
    ...
    if (!m_root) {
        ...
        while (current) {
            ...
            else { current->value = value; delete newNode; return; }
        }
        ...
        while (node->parent && node->parent->isRed) {
            ...
            if (node->parent == gp->left) {
                ...
                if (uncle && uncle->isRed) {
                    ...
                    gp->isRed = true; node = gp;
                } else {
                    if (node == node->parent->right) { node = node->parent; rotateLeft(node); }
                    node->parent->isRed = false; gp->isRed = true; rotateRight(gp);
                }
            } else {
                Node* uncle = gp->left;
                if (uncle && uncle->isRed) {
                    ...
                    if (node == node->parent->left) { node = node->parent; rotateRight(node); }
                    node->parent->isRed = false; gp->isRed = true; rotateLeft(gp);
                }
            }
        }
        m_root->isRed = false;
...
bool RedBlackTree8::remove(int key)
{
    ...
    m_timeSum += timer.elapsed();
    m_stats.avgProcessingTimeMs = m_timeSum / qMax(1, m_stats.totalInsertions + m_stats.totalDeletions);
    return true;
}
...
/** @brief 获取当前统计数据 */
RedBlackTree8::Stats RedBlackTree8::stats() const { return m_stats; }

/** @brief 重置所有统计数据为零值 */
void RedBlackTree8::resetStatistics() { m_stats = Stats{}; m_timeSum = 0.0; }
