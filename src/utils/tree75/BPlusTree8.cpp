 ...
 * @return 新版本号
 */
int BPlusTree8::insert(double key, int value, int version)
{
    QElapsedTimer timer;
    timer.start();
 ...
 */
void BPlusTree8::remove(double key, int version)
{
    QElapsedTimer timer;
    timer.start();
 ...
 * @return 是否存在
 */
bool BPlusTree8::contains(double key, int version) const
{
    QElapsedTimer timer;
    timer.start();
 ...
 */
int BPlusTree8::rank(double key, int version) const
{
    QElapsedTimer timer;
    timer.start();
 ...
 * @return 新的子树根
 */
BPlusTree8::PNode* BPlusTree8::insertNode(PNode* n, double key, int val)
{
    if (n == nullptr) {
 ...
 * @return 新节点（共享子节点）
 */
BPlusTree8::PNode* BPlusTree8::cloneNode(PNode* n)
{
 ...
 * @return 子树中的节点数
 */
int BPlusTree8::nodeCount(PNode* n) const
{
 ...
 */
void BPlusTree8::collectInOrder(PNode* n, QVector<QPair<double, int>>& elements)
{
    if (n == nullptr) return;
 ...
 * @return 平衡BST的根节点
 */
BPlusTree8::PNode* BPlusTree8::buildBalanced(
