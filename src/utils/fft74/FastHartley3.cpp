 ...
 * 使变换系数按零交叉数排列。
 */
QVector<double> FastHartley3::convolve_dummy(const QVector<double>& data) const
{
    int n = data.size();
    QVector<int> perm = dummy_perm(n);
 ...
 */
void FastHartley3::fwht(QVector<double>& data)
{
    int n = data.size();
    for (int h = 1; h < n; h <<= 1) {
 ...
 * 先进行位反转，再应用Gray码变换。
 */
QVector<int> FastHartley3::dummy_perm(int n) const
{
    QVector<int> perm(n);
    int bits = 0;
    int temp = n;
 ...
 * @return 功率谱（变换系数的平方）
 */
QVector<double> FastHartley3::powerSpectrum(const QVector<double>& data) const
{
 ...
 */
QVector<double> FastHartley3::autoCorrelation(const QVector<double>& data) const
{
    // 前向变换
    QVector<double> fwd = forward(data);
 ...
 * @return Walsh序号（过零次数）
 */
int FastHartley3::walshOrder(const QVector<double>& data) const
{
