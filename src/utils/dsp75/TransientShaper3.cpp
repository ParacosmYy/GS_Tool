...
#include "utils/dsp75/TransientShaper3.h"
 ...
 * @param parent 父对象指针
 */
TransientShaper3::TransientShaper3(QObject* parent)
    : QObject(parent)
{
}
 ...
 * @param hz LFO频率(Hz)
 */
void TransientShaper3::setRate(double hz)
{
 ...
 */
void TransientShaper3::setDepth(double ms)
{
 ...
 * @param v 声部数
 */
void TransientShaper3::setVoices(int v)
{
 ...
 */
void TransientShaper3::setFeedback(double fb)
{
 ...
 */
void TransientShaper3::setMix(double mix)
{
 ...
 * 使用线性插值读取延迟线避免量化噪声。
 */
QVector<double> TransientShaper3::process(const QVector<double>& input)
{
    QElapsedTimer timer;
    timer.start();
 ...
 * 产生去相关效果，增加合唱的丰富度。
 */
double TransientShaper3::lfo(double phase, int voice) const
{
    /* 正弦波LFO + 声部间微调 */
 ...
 */
QPair<double, double> TransientShaper3::delayRange() const
{
    double minDelay = 0.0;
 ...
 * @return 调制频率(Hz)
 */
double TransientShaper3::modulationFrequency() const
{
    return m_rate;
}
 ...
 * @brief 重置延迟线缓冲区
 */
void TransientShaper3::resetBuffers()
{
