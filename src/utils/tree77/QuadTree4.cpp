...
#include "utils/tree77/QuadTree4.h"
 ...
 * @param parent 父对象指针
 */
QuadTree4::QuadTree4(QObject* parent)
    : QObject(parent)
{
}
 ...
 * @return 新版本号
 */
int QuadTree4::insert(double key, int value, int version)
{
    QElapsedTimer timer;
    timer.start();
 ...
 */
void QuadTree4::remove(double key, int version)
{
    QElapsedTimer timer;
    timer.start();
 ...
 * @return 是否存在
 */
bool QuadTree4::contains(double key, int version) const
{
    QElapsedTimer timer;
    timer.start();
    ...
    const_cast<QuadTree4*>(this)->m_stats.totalQueries++;
    const_cast<QuadTree4*>(this)->m_timeSum += timer.elapsed();
    const_cast<QuadTree4*>(this)->m_stats.avgProcessingTimeMs =
 ...
 */
int QuadTree4::rank(double key, int version) const
{
    QElapsedTimer timer;
    timer.start();
 ...
 * @brief 重置统计信息
 */
void QuadTree4::resetStatistics()
{
    m_stats = Stats();
    m_timeSum = 0.0;
}
 ...
 * @return 新的子树根
 */
QuadTree4::PNode* QuadTree4::insertNode(PNode* n, double key, int val)
{
    if (n == nullptr) {
 ...
 * @return 新节点（共享子节点）
 */
QuadTree4::PNode* QuadTree4::cloneNode(PNode* n)
{
 ...
 * @return 子树中的节点数
 */
int QuadTree4::nodeCount(PNode* n) const
{
 ...
 */
void QuadTree4::collectInOrder(PNode* n, QVector<QPair<double, int>>& elements)
{
    if (n == nullptr) return;
 ...
 * @return 平衡BST的根节点
 */
QuadTree4::PNode* QuadTree4::buildBalanced(
