 ...
 * @param sr 采样率(Hz)
 */
void SpectralLeakage2::setSampleRate(double sr)
{
 ...
 * @param ms 攻击时间(ms)
 */
void SpectralLeakage2::setAttackTime(double ms)
{
 ...
 * @param ms 释放时间(ms)
 */
void SpectralLeakage2::setReleaseTime(double ms)
{
 ...
 */
void SpectralLeakage2::setMethod(const QString& method)
{
 ...
 * 然后应用攻击/释放平滑得到最终包络曲线。
 */
QVector<double> SpectralLeakage2::detect(const QVector<double>& signal)
{
    QElapsedTimer timer;
    timer.start();
 ...
 */
QVector<double> SpectralLeakage2::hilbertEnvelope(const QVector<double>& sig)
{
 ...
 * 信号上升时快速跟踪，下降时缓慢释放。
 */
QVector<double> SpectralLeakage2::peakEnvelope(const QVector<double>& sig)
{
    const int N = sig.size();
    QVector<double> env(N, 0.0);
