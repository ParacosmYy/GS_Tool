...
public:
    ...
    /** @brief 获取累计接收包次数 @return recordRxBytes调用次数(包级计数) */
    quint64 totalPacketsIn() const { return m_totalPacketsIn; }

    /** @brief 获取累计发送包次数 @return recordTxBytes调用次数(包级计数) */
    quint64 totalPacketsOut() const { return m_totalPacketsOut; }

    /** @brief 获取历史最高瞬时带宽(RX+TX, bytes/s) @return 峰值速率 */
