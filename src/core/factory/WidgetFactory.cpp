...
/**
 * @brief 注册一个新的控件类型及其创建函数
 * @param typeName 控件类型名称，作为创建时的查找键
 * @param creator 控件创建回调函数，接收父控件指针并返回新创建的控件实例
 */
...
/**
 * @brief 注销指定名称的控件类型，移除其创建函数
 * @param typeName 要注销的控件类型名称
 */
...
/**
 * @brief 根据类型名称创建控件实例
 * @param typeName 已注册的控件类型名称
 * @param parent 新控件的父控件指针
 * @return 成功时返回创建的控件指针，类型未注册时返回nullptr
 */
...
/**
 * @brief 检查指定名称的控件类型是否已注册
 * @param typeName 待检查的控件类型名称
...
/**
 * @brief 获取所有已注册的控件类型名称列表
 * @return 已注册类型名称的字符串列表
 */
