...
bool ShortcutManager::hasConflict(const QKeySequence &key, QString *out) const { for (const auto &s : m_shortcuts) if (s.currentKey == key && !key.isEmpty()) { if (out) *out = s.id; ++const_cast<ShortcutManager*>(this)->m_totalConflicts; return true; } return false; }
