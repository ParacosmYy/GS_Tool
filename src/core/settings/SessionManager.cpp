...
quint64 SessionManager::totalSaves() const
{
    return m_totalSaves;
}

/** @brief 获取会话加载总次数 @return 加载操作总次数 */
quint64 SessionManager::totalLoads() const
{
    return m_totalLoads;
}

/** @brief 获取会话操作错误总次数 @return 错误次数 */
quint64 SessionManager::errorCount() const
{
    return m_errorCount;
}
