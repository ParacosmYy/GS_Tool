...
/** @brief 析构函数，遍历所有连接依次close()+delete确保资源正确释放，连接对象没有设parent需要手动delete避免内存泄漏 */
...
/** @brief 创建指定类型的连接(委托给ConnectionFactory)，不传parent给工厂由ConnectionManager手动管理生命周期避免双重删除，创建成功后自动加入内部连接列表 @param type 连接类型枚举 @return 新创建的连接实例，失败返回nullptr */
...
/** @brief 串口便捷方法(内部委托给通用createConnection) @return 新创建的串口连接实例 */
IConnection* ConnectionManager::createSerialConnection()
{
    return createConnection(ConnectionType::Serial);
}

/** @brief 删除一个连接(自动close+deleteLater)，使用deleteLater确保所有已排队的信号槽执行完毕后再销毁对象，连接不在管理列表中时不做任何操作 @param conn 要删除的连接指针 */
...
/** @brief 按索引获取连接 @param index 连接索引 @return 连接指针，越界返回nullptr */
...
/** @brief 切换活跃连接到指定实例，目标连接必须已存在于活跃列表中否则不做任何操作 @param conn 目标连接指针 */
