...
void SmartAutoComplete::setDictionary(const QStringList &w) { m_dictionary = w; ++m_totalDictionaryUpdates; }
/** @brief 追加词汇到词典 @param w 词汇列表 */
void SmartAutoComplete::addWords(const QStringList &w) { m_dictionary.append(w); m_dictionary.removeDuplicates(); ++m_totalDictionaryUpdates; }
...
/** @brief 清空补全词典 */
void SmartAutoComplete::clearDictionary() { m_dictionary.clear(); ++m_totalDictionaryUpdates; }
/** @brief 按键事件处理 — Escape关闭弹窗 @param e 按键事件 */
void SmartAutoComplete::keyPressEvent(QKeyEvent *e) { if (e->key()==Qt::Key_Escape) { ++m_totalDismissals; hide(); } else QWidget::keyPressEvent(e); }

/** @brief 文本变化时执行前缀匹配并显示建议 @param text 当前输入文本 */
void SmartAutoComplete::onTextChanged(const QString &text) {
    if (text.length() < m_minChars) { if (isVisible()) ++m_totalDismissals; hide(); return; }
    QStringList results;
    ...
    if (results.isEmpty()) { ++m_totalDismissals; hide(); return; }
    ++m_totalTriggers;
