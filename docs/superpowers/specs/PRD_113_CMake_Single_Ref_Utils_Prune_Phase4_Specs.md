...
- 从主 GUI 目标 CMake 清单中移除第四批每目录 1 条 active 引用的真实命名 `utils` 候选目录。
- 保留源码，不删除、不移动。
...
| 文档 | `docs/prd/PRD_113_CMake_Single_Ref_Utils_Prune_Phase4.md` | 新增 |
| 文档 | `docs/superpowers/specs/PRD_113_CMake_Single_Ref_Utils_Prune_Phase4_Specs.md` | 新增 |
...
`avltree`, `bfgs`, `capture`, `crc_verifier`, `cycle`, `decomposer`, `gaussquad`, `golay`, `groebner`, `heap`, `hessenberg`, `hmm`, `inv_perm`, `lfu`, `lifting`, `lru`, `moment`, `movingmax`, `qrupdate`, `redblack`, `scaler`, `segment`, `skiplist`, `sliding`, `smoother`, `state`, `threshold`, `trie`, `trigger`, `twoway`, `warp`

## 7. 验收标准

- [ ] `python tools/project-audit/project_audit.py --limit 5` 显示 `duplicate entries: 0` 和 `missing listed files: 0`。
- [ ] 31 个目标目录在 active CMake refs 中归零。
...
| commit | 随本轮 `CMake: 移除第四批单引用utils候选` 提交记录 |
