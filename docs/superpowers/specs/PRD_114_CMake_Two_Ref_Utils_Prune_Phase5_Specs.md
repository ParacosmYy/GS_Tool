# PRD-114 - CMake Two Ref Utils Prune Phase 5
...
- 从主 GUI 目标 CMake 清单中移除第五批真实命名 2-ref `utils` 候选目录。
- 保留源码，不删除、不移动。
...
| 文档 | `docs/prd/PRD_114_CMake_Two_Ref_Utils_Prune_Phase5.md` | 新增 |
| 文档 | `docs/superpowers/specs/PRD_114_CMake_Two_Ref_Utils_Prune_Phase5_Specs.md` | 新增 |
...
`adams`, `adaptive_fft`, `agc`, `aho`, `anomaly`, `anova`, `arima`, `armodel`, `arnoldi`, `autocorr`, `autodiff`, `backtrack`, `bandmat`, `bayes`, `bernstein`, `bessel`, `bezier`, `bicgstab`, `bintree`, `bisect`, `bitalloc`, `bitfield`, `bitmap`, `bloom`, `bootstrap`, `bsp_tree`, `btree`, `butterfly`, `butterworth`, `cache`
...
| commit | 随本轮 `CMake: 移除第五批双引用utils候选` 提交记录 |
