...
### 3.2 Agent角色分配（11人团队，研发占比63.6%）

> **铁律**: 研发占比必须始终保持在 **60%~80%**，低于60%说明团队臃肿，代码框架将因研发力量不足而失效。
...
| 10 | **新特性设计者** | `requirements-analyst` | 挖掘工具能力缺口，提出新特性提案，撰写技术可行性分析，论证必要性（每3次迭代激活一次） | 研发 |
| 11 | **新特性评审员** | `system-architect` | 评审新特性提案的架构影响、开发成本、用户价值，决定通过/驳回/暂缓（每3次迭代激活一次） | 研发 |

**团队结构分析**:
- 研发 7人 (63.6%): 角色 2,3,4,5,6,10,11 — 架构+开发+特性设计评审的核心力量
- 管理 1人 (9.1%): 角色 1 — 需求管理
- 设计 1人 (9.1%): 角色 7 — UI/UX观感
- 质量 2人 (18.2%): 角色 8,9 — 代码审查+测试验证

> **扩编规则**: 任何新增角色必须优先考虑研发类别。如果新增非研发角色导致研发占比跌破60%，必须同时增加至少1个研发角色来平衡。
...
- 角色10(新特性设计者)和角色11(新特性评审员)每3次迭代激活一次，不参与常规迭代
...
### 3.4 新特性准入机制（每3次迭代评估一次）

> 核心理念: 工具永远不够好，功能永远可以更多。但盲目堆砌功能会导致代码膨胀和质量下降。
> 所以用"提案→评审→准入"的流程控制节奏，确保每个新特性都经过深思熟虑。

#### 3.4.1 两个新角色

| # | 角色 | Agent类型 | 职责 | 类别 |
|---|------|-----------|------|------|
| 10 | **新特性设计者** | `requirements-analyst` | 挖掘当前工具的能力缺口，提出新特性提案，撰写特性PRD，论证必要性和可行性 | 设计 |
| 11 | **新特性评审员** | `system-architect` | 评审新特性提案，评估架构影响、开发成本、用户价值，决定通过/驳回/暂缓 | 质量 |

#### 3.4.2 新特性评估节奏

每完成 **3次commit**（commit #3的倍数: #3, #6, #9, #12, ...），必须执行一次新特性评估:

```
┌───────────────────────────────────────────────────────────────────┐
│                每3次迭代: 新特性评估流程                            │
├───────────────────────────────────────────────────────────────────┤
│  Step 1: 新特性设计者 → 提交 1~3 个新特性提案                     │
│    ↓ 输出: docs/prd/FEATURE_xxx.md (含动机、方案、工作量估算)     │
├───────────────────────────────────────────────────────────────────┤
│  Step 2: 新特性评审员 → 逐项评审每个提案                          │
│    ↓ 评估维度:                                                    │
│      A. 用户价值: 是否解决真实痛点？有多少用户需要？               │
│      B. 架构影响: 是否破坏现有分层？新增多少类？耦合度如何？       │
│      C. 开发成本: 预估代码行数？需要几个迭代完成？                 │
│      D. 复用潜力: 新增的组件能否被其他特性复用？                   │
│    ↓ 结论: 通过 / 驳回 / 暂缓(下轮再议)                          │
├───────────────────────────────────────────────────────────────────┤
│  Step 3: 通过的提案 → 加入迭代计划，下次迭代开始实现              │
│         驳回的提案 → 记录驳回原因，归档到 docs/prd/rejected/      │
└───────────────────────────────────────────────────────────────────┘
```

**评估规则**:
- 每次评估最多通过 **1个** 新特性提案（控制膨胀速度）
...
| 优先级 | 特性名称 | 简述 | 参考标杆 | 预估复杂度 |
|--------|---------|------|---------|-----------|
| **P0** | CAN/CANFD 通信支持 | 通过USB-CAN适配器（如CANable、PCAN）收发CAN/CANFD帧，支持DBC数据库解析、总线监听、报文过滤 | CANoe, BUSMASTER | 高 |
| **P0** | 高级波形引擎 | 参考VOFA+的波形控件，实现实时FFT频谱分析、直方统计、X/Y scatter模式、多Y轴、游标测量、缩放平移 | VOFA+, Serial Studio | 高 |
| **P0** | JustFloat/FireWater协议 | 兼容VOFA+的JustFloat（小端浮点字节流）和FireWater（CSV尾标记）协议，支持直接对接现有嵌入式代码 | VOFA+ | 中 |
| **P1** | 数据录制回放增强 | 多数据流时间戳对齐回放（串口+RTT同步）、变速回放、书签标记、时间范围导出 | PulseView | 中 |
| **P1** | 脚本引擎 | 内嵌Lua/Python脚本，用户可编写自动化测试脚本（发送序列→等待响应→校验→报告） | Docklight | 高 |
...
| **Docklight** | 自动化测试脚本、Modbus预置模板、数据生成器 | 脚本引擎、协议模板参考 |
| **WindTerm** | SSH终端、SFTP文件管理、多Tab终端、会话管理 | 终端增强参考 |
| **Vercel Dashboard** | 数据展示清晰、状态指示优雅、进度条流畅 | UI/UX设计语言参考 |
...
5. **每3次迭代额外启动 Agent 10+11**: 新特性设计者提案 + 新特性评审员评审（见3.4节）
...
评分追踪已迁移至独立文档: [docs/tracking/SCORE_TRACKING.md](docs/tracking/SCORE_TRACKING.md)
