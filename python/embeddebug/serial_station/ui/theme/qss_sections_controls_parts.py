"""QSS 控件分区子助手 — 拆分 controls_section 的内容为按 /* === ... === */ 分区。

每个子助手返回一个 QSS 段（含分区注释头），由 ``controls_section`` 串联拼接。
...
def _led_part() -> str:
    return """/* 状态 LED：自绘圆形，QSS 仅控制容器背景。 */
...
def _button_part() -> str:
    return f"""/* Batch 7-5: RippleButton（ripple 自绘，QSS 背景透明让涟漪可见） */
...
def _gauge_value_part() -> str:
    return f"""/* 仪表盘：自绘，QSS 仅控制背景。 */
...
def _dashboard_part() -> str:
    return f"""/* === Dashboard（画布/控件库/标签页） === */
...
def _dashboard_panel_part() -> str:
    return f"""/* Batch 17: DashboardPanel 顶栏控件 + 主体容器 */
...
def _auxiliary_widgets_part() -> str:
    return """/* Batch 40: 新增微交互控件。控件均自绘主体（paintEvent），QSS 仅契约占位
