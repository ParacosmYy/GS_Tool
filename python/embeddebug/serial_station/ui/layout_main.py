...
def assemble_three_zone(
    ...
    _build_left_zone(owner, splitter, toolbar_layout, send_row_layout, inject_row_layout)
    log_card, waveform_card = _build_center_zone(owner, splitter)
    _build_right_zone(owner, splitter, log_row_layout, profile_row_layout, footer_layout)

    # 中区占主导：左 22% / 中 56% / 右 22%。
    splitter.setStretchFactor(0, 22)
    splitter.setStretchFactor(1, 56)
    splitter.setStretchFactor(2, 22)
...
def _build_left_zone(
    ...
    connection_card = wrap_layout(
        zone, toolbar_layout, title=owner.tr("Connection"), icon_name="plug"
    )
