...
- [x] v12 在 B26 中替换为当前默认，v11/v10/v9/v8 继续作为跨端回滚资产。
...
### Task B23: 个人体验入口 host fail-closed

**目标：** 防止用户通过环境变量误把个人本机账本暴露到局域网；个人入口和显式局域网预览保持不同的安全边界。

- [x] `windows/run.py` 仅规范化 IPv4 loopback，拒绝非 loopback、无效地址和当前入口不支持的 IPv6。
- [x] 启动在应用创建和数据库访问前拒绝危险 host 配置，并保留可操作的 `--lan-preview` 提示。
...
在保持本地优先和可直接体验的前提下，把 AI Token Tracker 交付为一个可持续演进的中心化产品：自动采集优先、前后端边界清晰、六个正式责任角色（UI-1/UI-2/UI-3、FE、BE、ARCH）按五个 UI 模块交付，网页和 Android 通过统一契约集成。视觉目标是参考 Moonshot 官网的空间感、黑底大排版和克制动效，并用 Material 3 的语义 token、可读性和可访问性把信息体验做得更完整。

## 正式角色基线

正式责任模型以 ADR-084 和 `roles/README.md` 为准；执行子轨道可以拆分任务，但不新增最终 owner。
...
- [x] 六个正式责任角色和 FE/ARCH 执行子轨道的交付模板存在；历史目录通过 ADR-084 映射。
...
### Task B26: Web/Android 场景资产 v12

**目标：** 解决 v11 登录首屏中角色脸部被认证卡片切断、设备层次不够清晰的问题，并继续维持跨端同像素资源和可读性边界。

- [x] 生成 v12 宽屏场景：左侧深海军蓝低细节文案安全区，右侧完整角色脸部、银色专业笔记本、独立桌面工作站和 Rust/RL 遥测屏幕。
- [x] Web 与 Android 使用相同 SHA-256 像素资产，默认引用切换到 v12；v11/v10/v9/v8 保留回滚。
...
**目标：** 在不破坏 v12 回滚链和左侧文字安全区的前提下，明确表达嵌入式 Rust/RL 构建场景，并让 Web、Android 与可分享 EXE 使用同一当前品牌资产。

- [x] 基于 v12 进行 v13 视觉迭代：成年御姐工程师、银色 MacBook Pro/Mac Studio 风格设备、Rust/RL 代码与遥测屏幕、左侧低细节安全区。
...
### Task B29: 已填充隔离数据库备份恢复演练

**目标：** 用真实写入过记录的隔离 SQLite 数据库验证备份、完整性校验、只读清单和 staging 恢复链路，避免只依赖空 schema 演练。

- [x] 通过 CLI 创建并校验隔离数据库备份，大小 `110592` bytes、完整性 `ok`、外键错误 `0`。
- [x] 使用 `backup-inventory --verify` 获得 1 个备份、0 个完整性失败，并恢复到明确 staging 目标后再次验证。
- [x] 新增 ADR-088，并同步发布就绪、验收矩阵、架构师复核和审计契约。
- [>] 真实中心数据、离线副本、保留周期、负责人签署和正式回滚演练仍待部署环境。
...
- [>] UI-3 已完成登录页、v8/v9/v10/v11/v12/v13 场景合成、首帧可读性和隔离空 schema 下仪表盘/管理员页的本地观察；真实 320px 设备指标及合法会话的焦点、错误态、导出和 API 失败路径仍待完成。
