...
| 后端工程师 | 鉴权、用户隔离、参数化 SQL、SSRF、provider usage、响应上限、限流、日志与缓存边界 | 通过（中心服务）；Gateway 进行中 | `web.py`/`providers.py`/`provider_projection.py`/`ingest_auth.py`/`db.py`/`events.py`/`csv_export.py`；health 200、未登录 summary 401、统一错误包络、安全响应头、凭据脱敏、provider 原始响应投影和外部采集 token digest/到期/撤销/固定 source 已验证；Gateway 的 loopback/独立访问令牌/上游边界已写入 ADR-054，源码实现和真实 provider 联调尚未完成。 |
| 架构师 | 依赖方向、API/ADR、七槽位边界、根入口、版本控制、依赖锁定和演进风险 | 通过（现有源码边界）；Gateway 契约进行中 | `docs/architecture.md`、`docs/api-contract.md`、ADR-003/032/033/034/035/036/037/038/039/040/041/042/043/044/045/046/047/048/049/050/051/052/053/054、`roles/`、`start.bat`/`run.py`；现有访问日志、Caddy 边缘预检、敏感 artifact 隔离、provider 响应投影、runtime lock、统一有界 CSV、Android Release HTTPS 门禁、liveness/readiness 探针、跨端 v6 资产、Kimi provider 预设、EXE 构建工具链锁定、只读发布审计、本地 Chart.js 供应链和 Usage Ingest 边界均有记录；Gateway 源码、重试队列与正式 HTTPS/Android/EXE/浏览器仍是门禁。 |
