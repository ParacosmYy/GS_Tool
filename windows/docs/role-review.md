...
- UI-1：通过真实滚动态定位到旧入口 CTA 亮色片段残留问题，修复后品牌栏下方的阅读焦点回到指标和图表内容。
- UI-2：只新增兼容 CSS；复用导航既有 `is-scrolled` 状态，不复制 sticky 监听逻辑，不改变 CTA 链接、周期交互或数据流；焦点和系统动效降级明确。
- UI-3：`390×844` 与 `1683×892` 的首屏/深滚计算样式、截图、周期控件和清洁日志已复核；合成滚动工具超时后通过只读页面状态完成确认。
- 后端：无后端、数据库、接口、认证、Provider、Key 或数据流改动。
- 架构师：变化限定在 `01-shell` legacy compatibility boundary，`ui-polish.css` 仍低于 1000 行，无新依赖、无跨层耦合。

## v129 观测区深蓝信号窗口

- UI-1：真实深滚截图定位到指标轨和图表卡片的材质断层；修复后观测区从透明轨平滑过渡到深蓝信号窗口，空态内容可扫描。
- UI-2：完整观测材质归属 `observatory-signal.css`，旧入口专属表现归属 `legacy-observatory.css`，通用 `ui-polish.css` 只保留 import 边界；未复制脚本、监听器或数据逻辑。
...
- UI-1：修复 390×844 首屏背景高亮侵入 `TOKEN SIGNAL`、说明文案和 CTA 阅读轨道的问题；移动场景焦点由默认 68% 收敛到 30%，左侧编辑留白连续，右侧叙事仍保留。
- UI-2：共享 `ui-polish.css` 同时覆盖 5000 旧启动链和 5011 完整启动链；320/768/1440 的动作位置、周期控件和桌面焦点保持；不改 DOM、脚本、图表或数据流。
- UI-3：真实 `5000` 的 390/320/768/1440、`5011` 的 390、透明顶栏、按钮 ARIA、无横向溢出和页面侧日志已复核；证据见 `ui-accessibility-evidence-v139.md`。
- 后端：无后端、数据库、接口、认证、Provider、Key 或数据流改动；保护服务未重启。
- 架构师：规则落在共享 scene presentation boundary，使用受控选择器覆盖完整链后置默认值，不引入 `!important`、重复模块或跨层耦合。

## v140 旧入口场景动效兼容补链

- UI-1：修复 5000 旧启动链静态背景与 5011 完整链动态背景不一致的问题；共享层增加低强度环境 glow/scan，避免旧入口首屏像静态海报。
- UI-2：新增信号只作用于装饰性 `.story-backdrop::after`，不改变布局、文字颜色、透明顶栏、DOM、脚本或数据流；完整链继续使用原有正式场景动画，不叠加同一伪元素的两套规则。
- UI-3：真实 5000 的 390/320/768/1440、5011 的 390/1440、动效计算值、透明顶栏、ARIA 快照、无横向溢出和页面侧日志已复核；证据见 `ui-accessibility-evidence-v140.md`。
- 后端：无后端、数据库、接口、认证、Provider、Key 或数据流改动；保护服务未重启。
- 架构师：兼容规则放在两个启动链共同加载的共享 scene presentation boundary，使用后置正式模块覆盖关系保持清晰，reduced-motion/forced-colors 边界完整。

## v141 工作信号表单标题轨道收口

- UI-1：修复移动端工作信号字段 `.optional` 提示被挤到第二行的问题；提示与字段标题同排，输入控件跨满下一行，字段高度从约 108px 收敛到 80px。
- UI-2：选择器限定在 `.activity-capture-card`，不影响自动采集、补录、登录或其它表单；桌面两列输入、备注 textarea 高度和业务 DOM 保持。
...
- UI-1：修复透明品牌行仍呈现横向材质带的视觉根因；共享兼容链移除 inset shadow，保留 signal line、文字 keyline 和焦点反馈。
- UI-2：`brand-clarity.css` 负责 5000/5011 兼容链，`brand-transparency.css` 负责完整链最终兜底；不新增模板、脚本、监听器、接口或业务耦合。
- UI-3：隔离 5026 的登录/注册 1440/390px，以及真实 5000/5011 桌面入口和 5000 移动入口已复核 computed style、无溢出和页面日志；证据见 `ui-accessibility-evidence-v148.md`。
- 后端：无后端、数据库、接口、认证、Provider、Key 或数据流改动；保护服务 PID `43832/8100` 未重启。
- 架构师：变化限定在透明材质边界，所有文本源码低于 1000 行，无新依赖；完整链与兼容链保持单一最终契约。

## v149 认证首帧自动聚焦层级

- UI-1：将自动聚焦从强 lime 键盘态降为 quiet lavender 首帧态，避免登录页刚打开就像错误态；用户首次 pointer/keyboard 交互后恢复完整焦点反馈。
- UI-2：`auth.js` 只维护既有 `data-auth-autofocus` 状态，`auth-focus.css` 只负责首帧视觉降级；不新增表单、接口、监听器之外的业务耦合。
...
- UI-1：降低认证卡片渐变 alpha、blur 与阴影，让角色、设备和蓝色环境光重新参与层次；输入框保留独立阅读底，文字不随透景变淡。
- UI-2：仅调整既有 `auth-signal.css` 的认证 presentation boundary；不新增模板、脚本、接口、会话或数据耦合，移动端表单优先级保持不变。
