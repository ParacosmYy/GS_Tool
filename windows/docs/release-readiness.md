...
| Web UI-1/UI-2 | pass（源码/本机视觉） | v12 场景、ADR-074 可见度基线、ADR-086 场景资产决策、显式主图层、动效和首帧可读性已在临时源码实例复核；v11/v10/v9/v8 保留回滚 |
...
| EXE | conditional | PyInstaller 6.22.0 已锁定并按 v12 资源重建 onedir EXE/ZIP；隔离 `/login`、v12 资源、`/api/v1/ready` 和 `%LOCALAPPDATA%` 建库已通过（ADR-082/087）；真实记录重启持久化、升级/回滚和签名仍待验收 |
