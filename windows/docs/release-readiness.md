...
| Web UI-3 | in progress | 登录页四档观察（v27/v29）、认证页 v23 唯一语义 h1/aria-hidden glitch 层、隔离合法会话下 Dashboard/Admin 空态、成员脱敏日志详情、动态错误 alert/status 播报、焦点回流、历史非空、服务端 CSV 200、Provider 502 错误态、个人 Activity 写入切片和 v14 默认浏览器首屏均已有源码/隔离证据；真实设备指标、浏览器下载落盘、真实 Provider 成功、reduced-motion/高对比度环境仍待证据 |
