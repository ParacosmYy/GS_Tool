...
| Web UI-1/UI-2 | pass（源码/本机视觉） | v11 场景、ADR-074 可见度基线、ADR-080 场景资产决策、显式主图层、动效和首帧可读性已在临时源码实例复核；v10/v9/v8 保留回滚 |
...
| EXE | conditional | PyInstaller 6.22.0 已锁定并生成 onedir EXE，`packaging/package.ps1` 可输出用户 ZIP；隔离 `/login`、资源加载和 `%LOCALAPPDATA%` 建库已通过（ADR-082）；真实记录重启持久化、升级/回滚和签名仍待验收 |
| Android APK | pending | 项目内 JDK 17、Gradle 9.5.0、command-line tools 和官方 Wrapper 已准备并由 doctor 识别；Android v11 背景与 SignalOrbit 已接入系统减少动画边界（ADR-079/080），API 37/Build Tools 因 SDK license 尚未交互确认仍 pending，APK/设备联调未验收（ADR-081） |
| HTTPS 中心部署 | pending | 应用生产绑定已强制 loopback（ADR-061）；Caddy v2.11.4 项目缓存入口、配置预检和显式 edge 启动器已接入（ADR-083），正式域名、ACL、证书、日志轮转和外部 health/ready 检查仍需在部署主机完成 |
