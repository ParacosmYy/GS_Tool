...
| EXE | conditional | PyInstaller 6.22.0 已锁定并按 v13 资源重建 onedir EXE/ZIP；v13 包 verifier、`/login`、`/api/v1/ready`、v13 资源 200、隔离 `%LOCALAPPDATA%` 建库和 EXE 目录无 data 已通过；v12→v13→v12 真实 API 升级/回滚与记录保留已通过（ADR-087/089/091/092）；签名入口/验证器已 fail-closed 接入（ADR-093），实际 Authenticode 和正式分发仍待验收 |
