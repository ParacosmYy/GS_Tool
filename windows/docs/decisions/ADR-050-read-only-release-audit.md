...
- 新增 `python -m token_tracker audit`，只检查关键交付文件、关键契约引用、源码 1000 行门禁、
  Windows/Android v6 资产 SHA-256、Python 运行时导入和外部工具链可用性。
...
- JSON 输出只包含检查名、状态和脱敏摘要，便于发布流水线保存证据而不暴露本地绝对路径或凭据。
