...
- `schema.py` 集中 SQLite DDL 和加法式迁移；`db.py` 集中连接、事务和查询，不再混合 schema 生命周期；
  `backup.py` 只负责本地文件备份、只读验证和有限清单，不把保留清理混入业务数据库用例。
...
- `csv_export.py` 统一个人与管理员 CSV 的 BOM、行数/字节预算和游标消费；查询模块只提供固定列投影。
- `/api/v1/health` 只做 liveness，`/api/v1/ready` 检查核心 SQLite schema；就绪失败使用 `503 SERVICE_NOT_READY`，不把内部异常返回客户端。
