...
**状态：** Android 第一条联动竖切片已完成；项目内 v14 场景和命令行工具链已接入，SDK license/APK 门禁单独跟踪
...
- 视觉资源：Windows 与 Android 共用的 `embedded-rust-engineer-bg-v14`，右侧为成年御姐气质、可爱但非性化的嵌入式 Rust/RL 工程师，手持银色 MacBook Pro 风格专业笔记本，工作台放置紧凑 Mac Studio 风格桌面工作站，后方屏幕使用 Rust 代码、RL 曲线和嵌入式遥测语义，左侧保留仪表盘留白；两端都使用渐变遮罩保证文字可读性，Android 背景增加低幅度漂移动画，v13/v12/v11/v10/v9/v8 及之前版本仍保留回滚，资产决策见 ADR-095。
