...
- 视觉资源：Windows 与 Android 共用的 `embedded-rust-engineer-bg-v6`，右侧为成年嵌入式 Rust/RL 工程师、银色 Pro 笔记本、桌面 Mac Studio 和实验板，左侧保留仪表盘留白；两端都使用渐变遮罩保证文字可读性，Android 背景增加低幅度漂移动画，v5 仍保留回滚。
