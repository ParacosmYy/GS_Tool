...
**状态：** Android 第一条联动竖切片已完成；本机工具链尚未安装
...
- 视觉资源：Windows 与 Android 共用的 `embedded-rust-engineer-bg-v10`，右侧为成年御姐气质、可爱但非性化的嵌入式 Rust/RL 工程师，手持银色专业笔记本，工作台放置紧凑银色桌面工作站、示波器与实验板，左侧保留仪表盘留白；两端都使用渐变遮罩保证文字可读性，Android 背景增加低幅度漂移动画，v9/v8 及之前版本仍保留回滚，资产决策见 ADR-076。
