...
| R-10 | EXE 便捷分发 | conditional | PyInstaller 6.22.0 lock、`packaging/build.ps1`/`package.ps1`、ADR-082/087、v12 EXE/ZIP SHA-256、隔离 `/login`/`ready`/资源 200 与 `%LOCALAPPDATA%` 建库证据 | 真实记录重启持久化、升级、回滚、签名和正式分发 |
...
3. EXE 真实记录重启持久化、升级/回滚、签名和正式分发验收；v12 构建、启动和默认用户目录证据已记录在 ADR-087，ADR-082 保留历史基线。
