...
| R-10 | EXE 便捷分发 | conditional | PyInstaller 6.22.0 lock、`packaging/build.ps1`/`package.ps1`、ADR-082/087/089/090/091、v13 EXE/ZIP SHA-256、包 manifest/verifier、隔离 `/login`/`ready`/v13 资源 200、真实记录重启读取与 `%LOCALAPPDATA%` 建库证据 | 真实升级、回滚、签名和正式分发 |
...
3. EXE v13 升级/回滚、签名和正式分发验收；v12 构建、启动、真实记录重启读取和默认用户目录证据已记录在 ADR-087，ADR-082 保留历史基线。
