...
| R-04 | Web 可视化趋势、模型占比、历史记录 | conditional | Dashboard、Chart.js 本地资产、v13 场景、UI-3 v17/v18 证据 | 真实设备指标、系统动效环境 |
...
| R-08 | Android 同账号联动 | conditional | Compose 登录、Remote/Repository/DTO/API v1 契约、v13 共享场景、项目内 JDK/Gradle/Wrapper、系统减少动画 UI 边界（ADR-079/090/081） | 用户接受 SDK license 后安装 API 37/Build Tools、APK 构建与授权设备安装联调 |
...
| R-10 | EXE 便捷分发 | conditional | PyInstaller 6.22.0 lock、`packaging/build.ps1`/`package.ps1`/`verify-upgrade-rollback.ps1`、ADR-082/087/089/090/091/092、v13 EXE/ZIP SHA-256、包 manifest/verifier、隔离 `/login`/`ready`/v13 资源 200、真实记录重启读取和 v12→v13→v12 数据保留证据 | Authenticode 签名和正式分发 |
...
3. EXE Authenticode 签名和正式分发验收；v12→v13→v12 升级/回滚、真实记录读取和用户目录保留已记录在 ADR-092，ADR-082 保留历史基线。
