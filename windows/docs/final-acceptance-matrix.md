...
- UI-1：移动端 `.optional` 不再单独换行；标题、提示与输入控件形成稳定的两层 field track，长表单的垂直扫描成本降低。
- UI-2：`label:has(.optional)` 规则限定在工作信号卡片，输入仍为 label 直接子节点，桌面两列、备注高度、键盘顺序和语义不变。
...
- UI-1：`≤360px` 且 `≤760px` 的短屏首屏保留 orbit 视觉锚点，同时把标题、主 CTA、次 CTA 和周期游标纳入一次扫描；装饰 cue 不与真实控件争夺高度。
- UI-2：`mobile-hero-scan.css` 独立拥有短屏规则；普通桌面/手机、认证、导航、接口和数据契约不受影响。
- UI-3：5026 的 `320×720`、`320×600`、`390×844`、默认桌面真实回归通过：周期按钮完整可见且 `tabIndex=0`，透明顶栏保持 `background=transparent`、无 blur/shadow，文档无正向横向扩张，应用日志 `[]`；证据见 `docs/ui-accessibility-evidence-v154.md`。
- 后端：无后端、认证、数据库、Provider、API 或数据契约变化；保护 PID `43832/8100` 未重启。
- 架构师：新增 CSS 模块高内聚、无新依赖或跨层耦合；所有修改文本源码低于 1000 行；Android、EXE、HTTPS/ACL 和真实设备发布门禁保持 pending。

## v155 认证阅读窗透景材质

- UI-1：认证页的卡片表面从偏重深色面收敛为低 alpha 蓝黑透景层，保留局部输入阅读底、标题对比度和提交动作层级。
- UI-2：`auth-signal.css` 继续独立拥有认证字段表现规则；模板、认证流程、移动端排序、reduced-motion 与 forced-colors 契约不变。
- UI-3：5027 的 1440/390/320px 登录与登录后 Dashboard 真实回归通过：卡片材质、48px 提交按钮、控件焦点、透明顶栏、周期按钮 `tabIndex=0`、文档宽度和应用日志 `[]` 均已复核；证据见 `docs/ui-accessibility-evidence-v155.md`。
- 后端：无后端、认证、数据库、Provider、API 或数据契约变化；保护 PID `43832/8100` 未重启。
